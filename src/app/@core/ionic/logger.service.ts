...
import { app_debug, define } from '../config.inc';
...
export class LoggerService {
    constructor(
        ...
        if (this.isFundebug()) {
            ...
            fundebug.releasestage = app_debug ? 'development' : 'production';
            // 如果暂时不需要使用Fundebug，将silent属性设为true
            fundebug.silent = !app_debug;
        }
    }
            ...
            fundebug.notifyError(err, {
                metaData: {
                    ...
                    user: {
                        ...
                        name: this.userService.userInfo.true_name,
                    },
                },
            });
    }
    ...
    alloyLeverInit() {
        AlloyLever.config({
            cdn:
                'http://s.url.cn/qqun/qun/qqweb/m/qun/confession/js/vconsole.min.js', //vconsole的CDN地址
